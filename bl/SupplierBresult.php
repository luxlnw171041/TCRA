...
//===========  อ่านข้อมูล
	...
	$stridno="";	
	if($_POST["txtidno"]){
		$stridno=" s_idno='".$_POST["txtidno"]."'";
	  }else {
			$stridno="";
			}
	$strname="";	
	if($_POST["txts_name"]){
		$strname=" s_name='".$_POST["txts_name"]."'";
	  }else {
			$strname="";
			}
	...
	$strsname="";		
	if($_POST["txts_sname"]){
		$strsname=" s_surname='".$_POST["txts_sname"]."' ";
	  }else {
			$strsname="";
		    }
	...
	if($stridno=="" && $strname=="" && $strsname=="") {
		...
		echo"window.location='SupplierBview.php';";
		echo"</script>";	
	}else {
	...
	if($stridno=="" && $strname!=="" && $strsname=="") {
		echo"<script language='JavaScript'>";
		echo"alert('กรุณาป้อนข้อมูล เลขประจำตัวผู้เสียภาษี');";
		echo"window.location='SupplierBview.php';";
		echo"</script>";
	}
	if($stridno=="" and $strname!=="" and $strsname!=="") {
		$strsql = "select * from Supplier where $strname AND $strsname";
	}
	if($stridno!=="" and $strname=="" and $strsname=="") {
		$strsql = "select * from Supplier where $stridno ";
	}
	if($stridno!=="" and $strname=="" and $strsname!=="") {
		$strsql = "select * from Supplier where $stridno AND $strsname";
	}
	if($stridno!=="" and $strname!=="" and $strsname=="") {
		$strsql = "select * from Supplier where $stridno AND $strname";
	}
	if($stridno!=="" and $strname!=="" and $strsname!==""){
		$strsql = "SELECT * FROM Supplier WHERE $stridno AND $strname AND $strsname";
	}
...
<div class="bigblock">
  ...
  <div class="block-c">
        ...
        <div class="menu"> <img src="../bl/images/mainmenu.jpg" width="239" height="55" />
          <div id="navside">
			<div class="sec-bmenu">
              ...
              <ul>
                <li class="current"><a href="custBview.php">ค้นหาข้อมูลลูกค้าที่มีประวัติ</a></li>
              </ul>
            </div>
	...
	//{
      ...
      ?>
      <div><center><img src="../picture/<?php echo $result["c_pic"];?>"></center></div><br />
      <table width="90%" border="1" cellpadding="5" cellspacing="0" bordercolor="#FF9933">
        <tr align="center" valign="middle" bgcolor="#FBB18E"> 
          ...
          <td><strong>ลักษณะการกระทำความผิด</strong></td>
          <td><strong>รายละเอียดการกระทำความผิด</strong></td>
          <td><strong>วันที่กระทำความผิด</strong></td>
        </tr>
        <tr align="left" valign="top"> 
        
          <td><?=$result["compname"];?></td>
          <td><?=$result["s_name"];?>&nbsp;&nbsp;&nbsp;<?=$result["s_surname"];?></td>
          <td align="center"><?=$result["s_idno"];?></td>
          <td align="left"><?=$result["demerit"];?></td>
          <td align="left"><?=$result["demeritdetail"];?></td>
          <td align="center"><?=$result["s_date"];?></td>
        </tr>
