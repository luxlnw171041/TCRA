...
counter=ReadString(formdata,Divider,5)
cp=5
nn=int(cp)  + int(counter)
'nn=cp
for i = int(cp)+2 to nn+1
		demerit=demerit&ReadString(formdata,Divider,int(i))&","
next
demeritdetail=ReadString(formdata,Divider,(int(nn)+2))
datedemerit=ReadString(formdata,Divider,(int(nn)+3))&"/"&ReadString(formdata,Divider,(int(nn)+4))&"/"&ReadString(formdata,Divider,(int(nn)+5))
pixfile1=ReadFileName(formdata,Divider,(int(nn)+6))
ShowName=ReadString(formdata,Divider,(int(nn)+7))
...
rs.open sql,cn,1,3
	rs.addnew
			...
			if pixfile1 <> "" then
					...
					rtrlength=(cint(dot)-(cint(slash)+1))
					file=mid(pixfile1,(cint(slash)+1))
					file2=mid(pixfile1,(cint(slash)+1),strlength)
...
'							Set fso=nothing
'						end if
						 strimage=id_driver&"-"&file2
						 'ext = SaveToFile(formdata,Divider,(int(nn)+5),"../image/upload/driver/"&strimage)
						 rs("image")=strimage&ext
						 rs.update
					end if 
			end if
rs.Close
...
<div style="background-image:url(images/bg-name.jpg); width:499px; height:77px;">
                ...
                <span style="font-weight:bold; color:#39F; font-size:18px; line-height:35px;">การเพิ่มข้อมูลพนักงานขับรถที่มีประวัติเสร็จสมบูรณ์แล้ว</span><br />
                กลับไปหน้าเพิ่มข้อมูลพนักงานขับรถที่มีประวัติ <span id="clickhere"><a href="driverBadd.asp">คลิกที่นี่</a></span> &nbsp;</span></div>
   	          </div>
       	      <p>&nbsp;</p>
       	      <p>&nbsp;</p>
       	      <p>&nbsp;</p>
