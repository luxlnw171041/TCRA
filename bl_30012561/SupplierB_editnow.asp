...
demerit=ReadString(formdata,Divider,15)
datedemerit=ReadString(formdata,Divider,16)&"/"&ReadString(formdata,Divider,17)&"/"&ReadString(formdata,Divider,18)
cimage=ReadString(formdata,Divider,19)
pixfile1=ReadFileName(formdata,Divider,20)
ShowName=ReadString(formdata,Divider,21)
If ShowName = "ShowName" then id_supplier=ReadString(formdata,Divider,22) else  id_supplier=ReadString(formdata,Divider,21) end if

Set cn=Server.CreateObject("ADODB.Connection")
cn.open Connect
Set rs=Server.CreateObject("ADODB.Recordset")
sql="select * from Supplier Where ID_Supplier="&id_supplier&" and ID_Member="&session("id_member")
rs.open sql,cn,1,3
	if not(rs.eof) then
			rs("company")=company
			rs("name")=name
			...
			if pixfile1 <> "" then
					...
					if pixfile1<>"" and cimage<>0 then
						if rs("image")<>"" then
							...
							iPath=Server.MapPath("../image/upload/Supplier/"&rs("image"))
						 ...
						 ext = SaveToFile(formdata,divider,(int(nn)+6),"../image/upload/Supplier/"&image)
						 rs("image")=image&ext
						 rs.update
					end if 
			end if
...
<div class="bigblock">
            ...
            <div class="subject">แก้ไขข้อมูล Supplier ที่มีประวัติ</div>
      ...
      <!-- InstanceEndEditable --><!-- InstanceBeginEditable name="content" -->
       	  <div class="content-data">
       	    <div class="welcome">
                ...
                <span style="font-weight:bold; color:#39F; font-size:18px; line-height:35px;">การแก้ไขข้อมูล&nbsp;Supplier&nbsp;ที่มีประวัติเสร็จสมบูรณ์แล้ว </span><br />
