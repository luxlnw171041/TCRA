...
telephone=ReadString(formdata,Divider,14)
mobile=ReadString(formdata,Divider,15)
counter=ReadString(formdata,Divider,16)
cp=16
...
If ShowName = "ShowName" then id_driver=ReadString(formdata,Divider,(int(nn)+8)) else  id_driver=ReadString(formdata,Divider,(int(nn)+7)) end if
...
sql="select * from Driver Where ID_Driver="&id_driver&" and ID_Member="&session("ID_Member")
'response.write(sql)
rs.open sql,cn,1,3
	if not(rs.eof) then
			rs("name")=name
			...
			if pixfile1 <> "" then
					...
					if pixfile1<>"" and cimage<>0 then
						if rs("image")<>"" then
						 ...
						 ext = SaveToFile(formdata,divider,(int(nn)+6),"../image/upload/driver/"&image)
						 rs("image")=image&ext
						 rs.update
					end if 
			end if
	...
	<div class="bigblock">
      ...
      <!-- InstanceEndEditable --><!-- InstanceBeginEditable name="content" -->
       	 ...
       	 <div style="background-image:url(images/bg-name.jpg); width:499px; height:77px;">
                ...
                <span style="font-weight:bold; color:#39F; font-size:18px; line-height:35px;">การแก้ไขข้อมูลพนักงานขับรถที่มีประวัติเสร็จสมบูรณ์แล้ว</span><br />
