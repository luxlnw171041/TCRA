...
idcard=ReadString(formdata,Divider,4)
sex=ReadString(formdata,Divider,5)
address=ReadString(formdata,Divider,6)&","&ReadString(formdata,Divider,7)&","&ReadString(formdata,Divider,8)&","&ReadString(formdata,Divider,9)&","&ReadString(formdata,Divider,10)
province=ReadString(formdata,Divider,11)
zipcode=ReadString(formdata,Divider,12)
...
cimage=ReadString(formdata,Divider,20)
pixfile1=ReadFileName(formdata,Divider,21)
ShowName=ReadString(formdata,Divider,22)
If ShowName = "ShowName" then id_customer=ReadString(formdata,Divider,23) else  id_customer=ReadString(formdata,Divider,22) end if

Set cn=Server.CreateObject("ADODB.Connection")
cn.open Connect
Set rs=Server.CreateObject("ADODB.Recordset")
sql="select * from Customer Where ID_Customer="&id_customer&" and ID_Member="&session("id_member")
rs.open sql,cn,1,3
	if not(rs.eof) then
			rs("company")=company
			rs("name")=name
			...
			if pixfile1 <> "" then
					...
					if pixfile1<>"" and cimage<>0 then
						if rs("image")<>"" then
						 ...
						 ext = SaveToFile(formdata,divider,(int(nn)+6),"../image/upload/customer/"&image)
						 rs("image")=image&ext
						 rs.update
					end if 
			end if
...
%>
                ...
                <span style="font-weight:bold; color:#39F; font-size:18px; line-height:35px;">การแก้ไขข้อมูลลูกค้าที่มีประวัติเสร็จสมบูรณ์แล้ว</span><br />
                กลับไปหน้าข้อมูลส่วนตัว <span id="clickhere"><a href="myaccount.asp">คลิกที่นี่</a></span> &nbsp;</span></div>
   	          </div>
       	      <p>&nbsp;</p>
       	      <p>&nbsp;</p>
       	      <p>&nbsp;</p>
