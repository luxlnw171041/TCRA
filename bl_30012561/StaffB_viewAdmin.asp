	...
	<div class="bigblock">
      ...
      <!-- InstanceEndEditable --><!-- InstanceBeginEditable name="content" -->
        ...
        <tr align="center" valign="middle" bgcolor="#FBB18E"> 
          <td width="22%"><strong>ชื่อ-นามสกุล</strong></td>
          <td width="33%"><strong>เลขที่บัตรประจำตัวประชาขน</strong></td>
          <td width="30%"><strong>ชื่อบริษัทที่บันทึกข้อมูล</strong></td>
          <td width="15%"><strong>วันที่บันทึก</strong></td>
        </tr>
        <%
				Set rs=Server.CreateObject("ADODB.Recordset")
				sql="select ID_Staff,Name,Surname,Telephone,StartDate,ID_Member from Staff order by StartDate DESC"
				rs.open sql,cn,1,3
						if not(rs.eof) then
        ...
        <tr align="left" valign="top"> 
          ...
          <%
				...
				rs1.open cn.Execute(sql1)
						if not(rs1.eof) then
			%>
          <td align="center"><a href="StaffB_detail.asp?id_Staff=<%=rs("id_Staff")%>"><%=rs1("Company")%></a></td>
			<%
						end if
				rs1.Close
				Set rs1=nothing
